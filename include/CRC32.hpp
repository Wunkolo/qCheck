...
) noexcept
{
	...
	for( std::size_t i = 0; i < 256; ++i )
	{
		...
		Table[11][i] = (Table[10][i] >> 8) ^ Table[0][std::uint8_t(Table[10][i])];
		Table[12][i] = (Table[11][i] >> 8) ^ Table[0][std::uint8_t(Table[11][i])];
		Table[13][i] = (Table[12][i] >> 8) ^ Table[0][std::uint8_t(Table[12][i])];
		Table[14][i] = (Table[13][i] >> 8) ^ Table[0][std::uint8_t(Table[13][i])];
		Table[15][i] = (Table[14][i] >> 8) ^ Table[0][std::uint8_t(Table[14][i])];
	}
...
std::uint32_t Checksum(RandomIterator First, RandomIterator Last, std::random_access_iterator_tag)
{
	...
	const __m512i ByteIndex = _mm512_set_epi8(
		...
		~0, ~0, ~0, 11, ~0, ~0, ~0, 10, ~0, ~0, ~0,  9, ~0, ~0, ~0,  8,
	...
	for( ; std::distance(First, Last) / 16; First += 16 )
	{
		...
		CRC =
			Table[15][std::uint8_t(InputLoLo      )] ^
